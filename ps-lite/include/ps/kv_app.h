...
void KVServer<Val>::Process(const Message& msg) {
  ...
  meta.request_data_size = msg.meta.request_data_size;
  meta.layer_index = msg.meta.layer_index;
  meta.server_rank = Postoffice::Get()->my_rank();
  KVPairs<Val> data;
  int n = msg.data.size();
  if (n) {
    CHECK_GE(n, 2);
...
void KVServer<Val>::Response(const KVMeta& req, const KVPairs<Val>& res) {
  ...
  msg.meta.request_data_size = req.request_data_size;
  msg.meta.layer_index = req.layer_index;
  msg.meta.server_rank = req.server_rank;
  if (res.keys.size()) {
...
void KVWorker<Val>::Send(int timestamp, bool push, bool pull, int cmd, const KVPairs<Val>& kvs) {
  ...
  for (size_t i = 0; i < sliced.size(); ++i) {
    ...
    msg.meta.layer_index = kvs.priority;
    ...
    msg.meta.request_data_size = msg.meta.data_size;
