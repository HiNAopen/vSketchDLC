...
void Van::PackMeta(const Meta& meta, char** meta_buf, int* buf_size) {
  ...
  pb.set_customer_id(meta.customer_id);
  pb.set_data_size(meta.data_size);
  pb.set_request_data_size(meta.request_data_size);
  pb.set_layer_index(meta.layer_index);
  pb.set_server_rank(meta.server_rank);
...
void Van::UnpackMeta(const char* meta_buf, int buf_size, Meta* meta) {
  ...
  meta->request_data_size = pb.request_data_size();
  meta->layer_index = pb.layer_index();
  meta->server_rank = pb.server_rank();
